...
function Inicio() {
  ...
  const funcionFetch = (link) => {
    ...
    fetch(link)
      ...
      .then((datosDeLaApi) => {
        setPersonajes(
          datosDeLaApi.results
        ); /* almaceno los datos de los personajes que me trae la api*/
        setPaginacion(
          datosDeLaApi.info

        );/* almaceno la info que me trae la api donde estan los 'next' y 'prev' */
        setCargando(false)
  ...
  return (
    <Fragment>
      <div className="container">
        ...
        {
          cargando ? <Spinner /> : <Tarjetas propiedad={personajes} nombreIngresado={texto} />
        }
        
        {
          !cargando && <Paginar prop1={atras} prop2={siguiente} prop3={paginacion} /> 
        }
